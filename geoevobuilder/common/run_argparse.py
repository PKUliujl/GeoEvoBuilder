...
def run_inputparameters():
    ...
    parser.add_argument('--purpose',type=int,choices=[0,1],help='0 is used for functional sequence design. Only activated at 1 when using additional MSA information (--MSA is given). default: 0',default=0)
    ...
    parser.add_argument('--ST',type=float,help='Sampling temperature (scaling factor) belongs to (0,1). Larger values increase the number of interation steps for convegence. default: 0.1', default = 0.1 )
    parser.add_argument('--n',type= int,help='The number of designed sequences. default: 10', default = 10)
    parser.add_argument('--SM',type=int,help='Starting mode with different sequence initializations (how many residues are randomly masked). default = 100 (%%) masked, i.e. random initialization)', default = 100)
    parser.add_argument('--Fixed',type= str,help='Those residues are expected to remain unchanged with names following the format either {residuenumber_chainid_resname,residuenumber_chainid_resname}, e.g. 1_A_W,10_A_C, or path to a file with one residue per line. Please note that the residuenumber is renumbered begining at 0.',default = None)
    ...
    parser.add_argument('--MSA',type=str,help='The distribution of sequence profile from MSA', default=None)
