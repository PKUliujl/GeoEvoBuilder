...
class ESM2finetuning_Net(torch.nn.Module):
    def __init__(self, mode=None):
        ...
        if mode == 'MSA':
            self.lin1 = torch.nn.Sequential(torch.nn.Linear(33 + 21, 64),
            ...
            self.lin1 = torch.nn.Sequential(torch.nn.Linear(33, 64),
                    torch.nn.BatchNorm1d(64),
                    torch.nn.ELU(),
                    torch.nn.Linear(64,256),
                    torch.nn.BatchNorm1d(256),
    ...
    def forward( self,x,edge_index,edge_attr,condition,triangle_nodes, triangle_edges,esm2in, MSA=None):
