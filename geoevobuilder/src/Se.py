...
class Net(torch.nn.Module):
    ...
    def forward(
        self, x, edge_index, edge_attr, condition, triangle_nodes, triangle_edges
    ):  # distanc
        ...
        edge_weight = self.edge_mlp(
            torch.cat((edge_attr1_1, edge_attr2_2, edge_attr3_3, edge_attr4_4, edge_attr5_5), 1)
        ).view(-1, 20, 20)
        ...
        o = torch.cat(
            (x1_1, x2_2, x3_3, x4_4, x5_5, condition_out, condition_triangle_out), 1
        )  # prior
...
class ESM2finetuning_Net(torch.nn.Module):
    def __init__(self, mode=None):
        ...
        self.MLP = torch.nn.Sequential(
            torch.nn.Linear(20 + 50 * 5 + 20 + 10 * 3 + 64 + 49 * 4, 100),
        ...
        self.MLP2 = torch.nn.Sequential(
            torch.nn.Linear(20 + 50 * 5 + 20 + 10 * 3 + 64 + 49 * 4, 300),
            torch.nn.BatchNorm1d(300),
            torch.nn.ELU(),
            torch.nn.Dropout(0.5),
            torch.nn.Linear(300, 49 * 4),
        )

    def forward(
        self, x, edge_index, edge_attr, condition, triangle_nodes, triangle_edges, esm2in, MSA=None
    ):
        seqP, rotamerP, intermediateP, intermediateEDGEP = self.Net(
            x, edge_index, edge_attr, condition, triangle_nodes, triangle_edges
        )

        if MSA is not None:
            condition = self.lin1(torch.cat((esm2in, MSA), 1))
        else:
            condition = self.lin1(esm2in)

        o = torch.cat((seqP, intermediateP, condition, rotamerP.view(-1, 49 * 4)), 1)
        o1 = self.MLP(o)
        o2 = self.MLP2(o)
        return o1  # , o2.view(-1, 49,4)
