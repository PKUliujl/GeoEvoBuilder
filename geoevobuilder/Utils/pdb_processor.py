    ...
    for model in structure:
        ...
        for chain in model:
            if chain.id == chainID:
                ...
                for i in range(int(interval_b), int(interval_e)):  # len(matrix)
                    ...
                    if pd.isna(matrix.iloc[i, -2]):
                        each_node_feature.extend([
                            ...
                            0,
                            0,
                            sin(matrix.iloc[i, -1] * 2),
                            sin(matrix.iloc[i, -1] * 3),
                            sin(matrix.iloc[i, -1] * 4),
                            cos(matrix.iloc[i, -1] * 2),
                        ...
                        each_node_feature.extend([
                            ...
                            0,
                            0,
                            sin(matrix.iloc[i, -2] * 2),
                            sin(matrix.iloc[i, -2] * 3),
                            sin(matrix.iloc[i, -2] * 4),
                            cos(matrix.iloc[i, -2] * 2),
                            cos(matrix.iloc[i, -2] * 3),
                            cos(matrix.iloc[i, -2] * 4),
                            0,
                            0,
                            0,
                            0,
                    ...
                    for ik in range(0, 13):
                        each_residue_position.extend([
                            sin((matrix.iloc[i, 3] - absolutepostion) / 1000 ** (ik / 13)),
                            cos((matrix.iloc[i, 3] - absolutepostion) / 1000 ** (ik / 13)),
                        ])
                    position_encoding.append(each_residue_position)
                    for CA in All_CA:
                        if (
                            CA.coord != chain[int(matrix.iloc[i, 3])]["CA"].coord
                        ).any() and CA.get_parent().get_parent().id == chain.id:
                            each_edge_attributes = []
                            each_edge_index = []
                            # print(chain[int(matrix .iloc[i,3])].id[1], CA.get_parent().id[1] )
                            CA_CA_dis = np.linalg.norm(
                                CA.coord - CA_coord
                            )  # distance between two CA
                            CA_CA_orientation = [
                                np.dot((CA.coord - CA_coord), CA_C) / np.linalg.norm(CA_C),
                                np.dot((CA.coord - CA_coord), CA_N) / np.linalg.norm(CA_N),
                            ...
                            CA_C_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["C"].coord - CA_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            CA_N_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["N"].coord - CA_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            CA_O_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["O"].coord - CA_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            if CA.get_parent().resname != "GLY":
                                ...
                                CA_CB_orientation = [
                                    np.dot(CA.get_parent()["CB"].coord - CA_coord, CA_C)
                                    / np.linalg.norm(CA_C),
                                    np.dot(CA.get_parent()["CB"].coord - CA_coord, CA_N)
                                    / np.linalg.norm(CA_N),
                                    np.dot(CA.get_parent()["CB"].coord - CA_coord, orthognal)
                                    / np.linalg.norm(orthognal),
                                ]
                            else:
                                CA_CB_dis = CA_CA_dis
                            ...
                            CA_CG1_orientation = [
                                np.dot(CA.get_parent()["CG1"].coord - CA_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG1"].coord - CA_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG1"].coord - CA_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            CA_CG2_dis = np.linalg.norm(CA.get_parent()["CG2"].coord - CA_coord)
                            CA_CG2_orientation = [
                                np.dot(CA.get_parent()["CG2"].coord - CA_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG2"].coord - CA_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG2"].coord - CA_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            ...
                            O_N_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["N"].coord - O_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            O_C_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["C"].coord - O_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            O_O_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["O"].coord - O_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            if CA.get_parent().resname != "GLY":
                                ...
                                O_CB_orientation = [
                                    np.dot(CA.get_parent()["CB"].coord - O_coord, CA_C)
                                    / np.linalg.norm(CA_C),
                                    np.dot(CA.get_parent()["CB"].coord - O_coord, CA_N)
                                    / np.linalg.norm(CA_N),
                                    np.dot(CA.get_parent()["CB"].coord - O_coord, orthognal)
                                    / np.linalg.norm(orthognal),
                                ]
                            else:
                                O_CB_dis = O_CA_dis
                            ...
                            O_CG1_orientation = [
                                np.dot(CA.get_parent()["CG1"].coord - O_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG1"].coord - O_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG1"].coord - O_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            O_CG2_dis = np.linalg.norm(CA.get_parent()["CG2"].coord - O_coord)
                            O_CG2_orientation = [
                                np.dot(CA.get_parent()["CG2"].coord - O_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG2"].coord - O_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG2"].coord - O_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            ...
                            N_O_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["O"].coord - N_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            N_C_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["C"].coord - N_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            N_N_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["N"].coord - N_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            if CA.get_parent().resname != "GLY":
                                ...
                                N_CB_orientation = [
                                    np.dot(CA.get_parent()["CB"].coord - N_coord, CA_C)
                                    / np.linalg.norm(CA_C),
                                    np.dot(CA.get_parent()["CB"].coord - N_coord, CA_N)
                                    / np.linalg.norm(CA_N),
                                    np.dot(CA.get_parent()["CB"].coord - N_coord, orthognal)
                                    / np.linalg.norm(orthognal),
                                ]
                            else:
                                N_CB_dis = N_CA_dis
                            ...
                            N_CG1_orientation = [
                                np.dot(CA.get_parent()["CG1"].coord - N_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG1"].coord - N_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG1"].coord - N_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            N_CG2_dis = np.linalg.norm(CA.get_parent()["CG2"].coord - N_coord)
                            N_CG2_orientation = [
                                np.dot(CA.get_parent()["CG2"].coord - N_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG2"].coord - N_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG2"].coord - N_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            ...
                            C_C_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["C"].coord - C_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            C_N_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["N"].coord - C_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            C_O_orientation = [
                                ...
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["O"].coord - C_coord, CA_N)
                                / np.linalg.norm(CA_N),
                            ...
                            if CA.get_parent().resname != "GLY":
                                ...
                                C_CB_orientation = [
                                    np.dot(CA.get_parent()["CB"].coord - C_coord, CA_C)
                                    / np.linalg.norm(CA_C),
                                    np.dot(CA.get_parent()["CB"].coord - C_coord, CA_N)
                                    / np.linalg.norm(CA_N),
                                    np.dot(CA.get_parent()["CB"].coord - C_coord, orthognal)
                                    / np.linalg.norm(orthognal),
                                ]
                            else:
                                C_CB_dis = C_CA_dis
                            ...
                            C_CG1_orientation = [
                                np.dot(CA.get_parent()["CG1"].coord - C_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG1"].coord - C_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG1"].coord - C_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            C_CG2_dis = np.linalg.norm(CA.get_parent()["CG2"].coord - C_coord)
                            C_CG2_orientation = [
                                np.dot(CA.get_parent()["CG2"].coord - C_coord, CA_C)
                                / np.linalg.norm(CA_C),
                                np.dot(CA.get_parent()["CG2"].coord - C_coord, CA_N)
                                / np.linalg.norm(CA_N),
                                np.dot(CA.get_parent()["CG2"].coord - C_coord, orthognal)
                                / np.linalg.norm(orthognal),
                            ]
                            ...
                            if chain[int(matrix.iloc[i, 3])].resname != "GLY":
                                CB_C_dis = np.linalg.norm(
                                    ...
                                    - chain[int(matrix.iloc[i, 3])]["CB"].coord
                                )
                                CB_C_orientation = [
                                    np.dot(
                                        CA.get_parent()["C"].coord
                                ...
                                CB_N_dis = np.linalg.norm(
                                    CA.get_parent()["N"].coord
                                    - chain[int(matrix.iloc[i, 3])]["CB"].coord
                                )
                                CB_N_orientation = [
                                    np.dot(
                                ...
                                CB_O_dis = np.linalg.norm(
                                    CA.get_parent()["O"].coord
                                    - chain[int(matrix.iloc[i, 3])]["CB"].coord
                                )
                                CB_O_orientation = [
                                    np.dot(
                                ...
                                CB_CA_dis = np.linalg.norm(
                                    CA.get_parent()["CA"].coord
                                    - chain[int(matrix.iloc[i, 3])]["CB"].coord
                                )
                                CB_CA_orientation = [
                                    np.dot(
                                ...
                                if CA.get_parent().resname != "GLY":
                                    CB_CB_dis = np.linalg.norm(
                                        CA.get_parent()["CB"].coord
                                        - chain[int(matrix.iloc[i, 3])]["CB"].coord
                                    )
                                    CB_CB_orientation = [
                                        np.dot(
                                ...
                                if CA.get_parent().resname != "GLY":
                                    CB_CB_dis = np.linalg.norm(
                                        CA.get_parent()["CB"].coord - CA_coord
                                    )
                                    CB_CB_orientation = [
                                        np.dot(CA.get_parent()["CB"].coord - CA_coord, CA_C)
                                        / np.linalg.norm(CA_C),
                            ...
                            CB_CG1_dis = np.linalg.norm(
                                CA.get_parent()["CG1"].coord
                                - chain[int(matrix.iloc[i, 3])]["CB"].coord
                            )
                            CB_CG1_orientation = [
                                np.dot(
                            ...
                            CB_CG2_dis = np.linalg.norm(
                                CA.get_parent()["CG2"].coord
                                - chain[int(matrix.iloc[i, 3])]["CB"].coord
                            )
                            CB_CG2_orientation = [
                                np.dot(
                            ...
                            each_edge_attributes.extend([
                                sin(matrix.iloc[endnode, 3] - matrix.iloc[i, 3]),
                                cos(matrix.iloc[endnode, 3] - matrix.iloc[i, 3]),
                            ])
                            edge_attributes.append(each_edge_attributes)
                ...
                assert len(node_feature) == len(position_encoding), (
                    "Wrong dimensional number when processing this pdbfile, please carefully check the pdbfile"
                )

                return {
                    "Node_features": np.array(node_feature),
            ...
            if chain.id == chainID:
                ...
                for i in range(int(interval_b), int(interval_e)):  # len(matrix)
                    ...
                    All_CA = [
                        ...
                        for atom in NA
                        if atom.id == "CA"
                        and (atom.coord != chain[int(matrix.iloc[i, 3])]["CA"].coord).any()
                        and atom.get_parent().get_parent().id == chain.id
                    ]  # atom.get_parent().id[1]
                    CA_coords = []
                    endnode = []
                    for CA in All_CA:
                        endnode.append(
                            matrix[matrix.iloc[:, 3] == CA.get_parent().id[1]].iloc[0, 0]
                        )
                ...
                return {
                    "triangleindex": np.array(triangle_index_total),
                    "triangle2line": np.array(triangle2line_total),
                }
...
def batch_fea(files, chainID, seqlength):
    ...
    assert Node_features.shape[0] == seqlength, (
        "Sequence length does not match the shape of protein nodes"
    )
